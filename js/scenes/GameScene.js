  ...
  _tickEnemyWorker(u, time, dt) {
    ...
    if (u.targetNode) {
      ...
      if (!this.moveToward(u, n.x, n.y, UDEF.worker.gatherRange, dt)) {
        ...
        if (u.workProgress >= 25.0) { // Increased from 10.0 to 25.0 for much longer harvesting time
  ...
  tickWorker(u, time, dt) {
    ...
    if (ENABLE_PROACTIVE_AI && u.role === null && u.age >= 2 && !u.moveTo) {
       ...
       const needs = [
         ...
         { type: 'granary', urgency: (this.resources.food >= this.storageMax.food && count('granary') < Math.ceil((count('farm') + count('garden')) / 4) + 1) ? 9 : 0 },
